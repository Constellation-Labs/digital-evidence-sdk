...
export const description = "Verify the Merkle Patricia Trie inclusion proof for a fingerprint. Fetches the proof, validates its structure, and checks it against the batch root.";
...
export function register(client) {
    return async (args) => {
        ...
        if (!fp) {
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ valid: false, message: `Fingerprint not found: ${args.hash}` }, null, 2),
                    },
                ],
                isError: true,
            };
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            valid: false,
                            message: "Fingerprint is not yet assigned to a batch — no proof available",
                        }, null, 2),
                    },
                ],
            };
        ...
        const hasValidStructure = proof.leafKey &&
            proof.leafValue &&
            proof.root &&
...
//# sourceMappingURL=verify-proof.js.map